...
void TinyPBCoder::decode(std::vector<AbstractProtocol::s_ptr>& out_messages, TcpBuffer::s_ptr buffer) {
  while(1) {
    std::cout<<"i am here in decode in TinyPBCoder"<<std::endl;
    ...
    int i = 0;
    std::cout<<"buffer->readIndex() is "<<buffer->readIndex()<<std::endl;
    std::cout<<"buffer->writeIndex() is "<<buffer->writeIndex()<<std::endl;
    ...
    if (i >= buffer->writeIndex()) {
      //std::cout<<"parse_success is"<<parse_success<<std::endl;
    ...
    if (parse_success) {
      ...
      std::cout << "parse method_name=" << message->m_method_name << std::endl;

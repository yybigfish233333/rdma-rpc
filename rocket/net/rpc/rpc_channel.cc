...
namespace rocket
{
    ...
    google::protobuf::Message* response, google::protobuf::Closure* done)
  {
    ...
    std::cout << "rpc callmethod" << std::endl;
    ...
    m_client = std::make_shared<TcpClient>(m_peer_addr);//这个m_peer_addr是服务器的地址
    ...
    // 这是把request序列化为req_protocol->m_pb_data
    ...
    m_client->connect([req_protocol, this]() mutable
      {
        ...
        //得等连接成功后才能建立qp
        resources* temp_res=m_client->getRes();
        std::cout<<"in connect,temp_res->sock="<<temp_res->sock<<std::endl;
        m_client->getConnection()->connect_qp(m_client->getRes());
        getTcpClient()->writeMessage(req_protocol, [req_protocol, this, my_controller](AbstractProtocol::s_ptr) mutable
          {
            ...
            getTcpClient()->readMessage(req_protocol->m_msg_id, [this, my_controller](AbstractProtocol::s_ptr msg) mutable
              {
                std::cout<<"in readMessage"<<std::endl;                  

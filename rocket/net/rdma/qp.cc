...
qp::qp(std::shared_ptr<rocket::pd> pd, std::shared_ptr<cq> cq,
       std::shared_ptr<srq> srq)
    : qp(pd, cq, cq, srq) {}

qp::qp(std::shared_ptr<rocket::pd> pd, std::shared_ptr<cq> recv_cq,
       std::shared_ptr<cq> send_cq, std::shared_ptr<srq> srq)
...
void qp::rtr(uint16_t remote_lid, uint32_t remote_qpn, uint32_t remote_psn) {
  ...
  // 在 RoCE 环境下启用全局路由头
  qp_attr.ah_attr.is_global = 1; // 启用 GRH
  qp_attr.ah_attr.dlid = 0;      // RoCE 环境下设置为 0
  ...
  // 逐字节填入远程 GID 信息
  remote_gid.raw[0] = 0xfe;
  remote_gid.raw[1] = 0x80;
  remote_gid.raw[2] = 0x00;
  remote_gid.raw[3] = 0x00;
  remote_gid.raw[4] = 0x00;
  remote_gid.raw[5] = 0x00;
  remote_gid.raw[6] = 0x00;
  remote_gid.raw[7] = 0x00;
  remote_gid.raw[8] = 0x02;
  remote_gid.raw[9] = 0x50;
  remote_gid.raw[10] = 0x56;
  remote_gid.raw[11] = 0xff;
  remote_gid.raw[12] = 0xfe;
  remote_gid.raw[13] = 0x8e;
  remote_gid.raw[14] = 0x5b;
  remote_gid.raw[15] = 0xa5;

  // 配置 GID 信息
  std::memcpy(&qp_attr.ah_attr.grh.dgid, &remote_gid,
              sizeof(remote_gid)); // 复制 GID 到 dgid
  qp_attr.ah_attr.grh.flow_label = 0;
  qp_attr.ah_attr.grh.hop_limit = 1;  // 单跳
  qp_attr.ah_attr.grh.sgid_index = 0; // 设置本地 GID 索引
